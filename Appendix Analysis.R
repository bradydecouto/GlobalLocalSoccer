...
mod1 <- (lmer(P_Avg_Hem ~ Skill * Condition  * Hemisphere  + Epoch +
                (1|subID) + (1|Condition:subID) + (1|Hemisphere:subID) + (1|Epoch:subID), 
              REML = FALSE, data = filter(Alpha_Hem)))
anova_stats(mod1)

mod1 <- (lmer(P_Avg_Hem ~ Skill * Condition  * Hemisphere  +
                (1|subID) + (1|Condition:subID) + (1|Hemisphere:subID), 
              REML = FALSE, data = filter(Alpha_Hem, Epoch == "E1")))
anova_stats(mod1)
...
mod1 <- (lmer(P_Avg_Hem ~ Skill * Condition  * Hemisphere  + Epoch +
                (1|subID) + (1|Condition:subID) + (1|Hemisphere:subID) + (1|Epoch:subID), 
              REML = FALSE, data = filter(Beta_Hem)))
anova_stats(mod1)

mod1 <- (lmer(P_Avg_Hem ~ Skill * Condition  * Hemisphere  +
                (1|subID) + (1|Condition:subID) + (1|Hemisphere:subID), 
              REML = FALSE, data = filter(Beta_Hem, Epoch == "E1")))
anova_stats(mod1)
...
mod1 <- (lmer(P_Avg_Hem_r ~ Skill * Condition  * Hemisphere  + Epoch +
                (1|subID) + (1|Condition:subID) + (1|Hemisphere:subID) + (1|Epoch:subID), 
              REML = FALSE, data = filter(Rel_Hem)))
anova_stats(mod1)

mod1 <- (lmer(P_Avg_Hem_r ~ Skill * Condition  * Hemisphere  +
                (1|subID) + (1|Condition:subID) + (1|Hemisphere:subID), 
              REML = FALSE, data = filter(Rel_Hem, Epoch == "E1")))
anova_stats(mod1)
...
mod1 <- (lmer(P_Avg_Hem_r ~  Condition   * Hemisphere +
                (1|subID) + (1|Condition:subID) + (1|Hemisphere:subID), 
              REML = FALSE, data = filter(LSkill_Rel)))
anova_stats(mod1)

      mod1 <- (lmer(P_Avg_Hem_r ~ Hemisphere    +
                      (1|subID) , 
                    REML = FALSE, data = filter(LSkill_Rel, (Condition == "N" ))))
      anova_stats(mod1)
      
      mod1 <- (lmer(P_Avg_Hem_r ~ Hemisphere    +
                      (1|subID) , 
                    REML = FALSE, data = filter(LSkill_Rel, (Condition == "O" ))))
      anova_stats(mod1)
      
      mod1 <- (lmer(P_Avg_Hem_r ~ Hemisphere    +
                      (1|subID) , 
                    REML = FALSE, data = filter(LSkill_Rel, (Condition == "B" ))))
      anova_stats(mod1)
...
mod1 <- (lmer(P_Avg_Hem_r ~ Condition  * Hemisphere+ 
              ...
              data = filter(LSkill_Rel, Epoch == "E2" & Skill == "LS")))
anova_stats(mod1)
...
mod1 <- (lmer(P_Avg_Hem_r ~  Condition   * Hemisphere +
                (1|subID) + (1|Condition:subID) + (1|Hemisphere:subID), 
              REML = FALSE, data = filter(Skill_Rel)))
anova_stats(mod1)
...
Rel_Plot <- Rel_Hem %>%
  group_by(subID, Epoch, Condition, Hemisphere, Skill) %>%
...
ggplot(filter(Rel_Plot, Epoch == "E1" & Skill == "LS"))+
  geom_boxplot(aes(Hemisphere, P_Avg, fill = Hemisphere),
               outlier.shape = NA)+
  ...
  ggtitle("Less-skilled Players")+
  scale_fill_manual(values = c("Left" = "grey95",
                               "Right" = "grey60"))
 


ggplot(filter(Rel_Plot, Epoch == "E1" & Skill == "S"))+
  geom_boxplot(aes(Hemisphere, P_Avg, fill = Hemisphere),
               outlier.shape = NA)+
  geom_point(aes(Hemisphere, P_Avg), alpha = .5)+  
  geom_line(aes(Hemisphere, P_Avg, group = subID), alpha = .3, color = "black")+
...
mod1 <- (lmr(P_Avg_Hem ~  Skill * Condition  *Hemisphere*Epoch+
